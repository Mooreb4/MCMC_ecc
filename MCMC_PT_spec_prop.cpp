...
// g++ -I/Users/blakemoore/eigen -I/Users/blakemoore/fftw/include -I/Users/blakemoore/Desktop/Grad\ Research/MCMC_ecc -I/Users/blakemoore/gsl/include -O3 -ffast-math -g3 -c -fmessage-length=0 -std=c++14 -MMD -MP -MF"MCMC_PT_spec_prop.d" -MT"MCMC_PT_spec_prop.o" -o "MCMC_PT_spec_prop.o" "MCMC_PT_spec_prop.cpp"
//build with
// g++ -L/Users/blakemoore/gsl/lib -L/Users/blakemoore/fftw/lib -o "tstmc_v2"  ../Amps.o ../TaylorF2e.o ../llhood_maxd.o ../fisher.o ./MCMC_PT_spec_prop.o  -lfftw3 -lgsl -lgslcblas -lm
...
//string path_samples = "/mnt/lustrefs/work/blake.moore2/MCMC_ecc/spec_prop/samples/";
...
string path_samples = "/Users/blakemoore/Desktop/Grad Research/MCMC_ecc/spec_prop/samples_DB/";
...
void spec_prop(vector<double> &loc, vector<double> &prop, const gsl_rng * r);
...
double newt_p_e(double e0, double p0, double e);
...
double cond_root(double plim, double e0, double p0, double ehere){
    return plim - newt_p_e(e0, p0, ehere);
}
double root_find_e(double plim, double e0, double p0){
    double e_1 = e0;
    double e_0;
    if(plim < p0){
        e_0 = e0 - e0*0.1;
    ...
    double cond_val_e_1 = cond_root(plim, e0, p0, e_1);
    double cond_val_e_0 = cond_root(plim, e0, p0, e_0);
    double e_2 = e_1 - cond_val_e_1*(e_1-e_0)/(cond_val_e_1-cond_val_e_0);
    if (e_2 < 0){e_2 = 0.01;}
    
    while(abs(cond_val_e_1) > 0.1){
        e_0 = e_1;
        cond_val_e_0 = cond_val_e_1;
        e_1 = e_2;
        cond_val_e_1 = cond_root(plim, e0, p0, e_1);
...
void spec_prop(vector<double> &loc, vector<double> &prop, const gsl_rng * r){
    double b = root_find_e(150, loc[2], loc[3]);
    if (b > 0.8) {b = 0.8;}
    double a = 0.0001;
    double e_draw = gsl_ran_flat(r, a, b);
    double p_draw = newt_p_e(loc[2], loc[3], e_draw) + gsl_ran_gaussian(r, 2);
...
double newt_p_e(double e0, double p0, double e){
    return p0*pow((e/e0),12./19.)*pow((1+121./304.*e*e)/(1+121./304.*e0*e0),870./2299.);
}
void write_to_DE(chain &c){
    if(c.DE_track == 1000){
        c.DE_track = 0;
...
void jump(vector<double> &loc, vector<double> &prop, SelfAdjointEigenSolver<Eigen::MatrixXd> &es, const gsl_rng * r, double &likehere, double &likeprop, double &rh, double T, double fend, double df, vector<complex<double>> &h2, vector<double> &noise, int &counter, int N_DE_samples, vector<vector<double>> &DE_samples){
    ...
    } else if (jump_roll > 0.15){
        if(N_DE_samples < 2) {fisher_prop(loc, prop, es, r);}
        else {DE_prop(loc, prop, r, N_DE_samples, DE_samples);}
    } else if (jump_roll > 0.05){
//        spec_prop(loc, prop, r);
//        double hast_fact = -1./(2.*2*2)*(pow((loc[3] - newt_p_e(prop[2], prop[3], loc[2])),2) - pow((prop[3] - newt_p_e(loc[2], loc[3], prop[2])),2));
        fisher_prop(loc, prop, es, r);
    } else {
       prior_prop(prop, r);
    }
...
//    cout_vec(prop);
    ...
    if(check_pri != 1){
        ...
        rh = min(1., exp(likeprop - likehere + hast_fact));
        if(rh >= urn){
            set_loc(prop, loc);
            likehere = likeprop;
            counter++;
